    ...
    Begin
      ...
      Begin
        ...
        Begin
          If bGrad Then
            p[x * 4 + 2 - d] :=
              Byte(min(255, max(0, round((v.get_Grad(x, y, d) - mm.minv) / mm.dv* 255))))
          Else
            p[x * 4 + 2 - d] :=
              Byte(min(255, max(0, round((v.get(x, y, d) - mm.minv) / mm.dv* 255))));
        End;
        p[x * 4 + 3] := 0;
      End
    End
  ...
  If mm.dv <> 0 Then
    ...
    Begin
      ...
      Begin
        If bGrad Then
          bVal := Byte
            (min(255, max(0, round(((v.get_Grad(x, y, iDepth) - mm.minv) / mm.dv) * 255))))
        Else
          bVal := Byte
