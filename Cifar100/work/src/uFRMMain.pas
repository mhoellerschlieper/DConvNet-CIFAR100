...
Begin
  Try
    ...
    if not LoadDefinition then
    begin

      // Die Layer-Definition......
      { Layer_Def.Clear;
        Layer_Def.ADD(CreateOpt_Input('input', 32, 32, 3));
        Layer_Def.ADD(CreateOpt_Conv('conv', 5, 50, 1, 2, 'relu'));
        Layer_Def.ADD(CreateOpt_Pool('pool', 1, 1));
        Layer_Def.ADD(CreateOpt_Conv('conv', 5, 40, 1, 2, 'relu'));
        Layer_Def.ADD(CreateOpt_Pool('pool', 2, 2));
        Layer_Def.ADD(CreateOpt_Conv('conv', 5, 30, 1, 2, 'relu'));
        Layer_Def.ADD(CreateOpt_Pool('pool', 2, 2));
        Layer_Def.ADD(CreateOpt_Conv('conv', 5, 20, 1, 2, 'relu'));
        Layer_Def.ADD(CreateOpt_Pool('pool', 2, 2));
        Layer_Def.ADD(CreateOpt_Hidden('softmax', 'softmax', 100, 'NONE'));
        Net.makeLayers(Layer_Def); }

      { Layer_Def.Clear;
        Layer_Def.ADD(CreateOpt_Input('input', 32, 32, 3));
        Layer_Def.ADD(CreateOpt_Conv('conv', 5, 20, 1, 1, 'relu'));
        Layer_Def.ADD(CreateOpt_Pool('pool', 1, 1));
        Layer_Def.ADD(CreateOpt_Conv('conv', 5, 20, 1, 1, 'relu'));
        Layer_Def.ADD(CreateOpt_Pool('pool', 2, 2));
        Layer_Def.ADD(CreateOpt_Hidden('softmax', 'softmax', 100, 'NONE'));
      ...
      // STRIDE: fasst Pixel zusammen
      // PAD   : Randbehandlung
      Layer_Def.Clear;
      Layer_Def.ADD(CreateOpt_Input('input', 32, 32, 3));
      Layer_Def.ADD(CreateOpt_Conv('conv 1', 3, 10, 2, 0, 'relu'));
      Layer_Def.ADD(CreateOpt_Pool('pool', 2, 2));

      Layer_Def.ADD(CreateOpt_Conv('conv 2', 5, 25, 2, 0, 'relu'));
      Layer_Def.ADD(CreateOpt_Pool('pool', 2, 2));
      // kleine Details, 3x3 Filter, Gradienten

      Layer_Def.ADD(CreateOpt_Conv('conv 3', 7, 25, 2, 7, 'relu'));
      Layer_Def.ADD(CreateOpt_Pool('pool', 2, 2));

      Layer_Def.ADD(CreateOpt_Conv('conv 4', 11, 25, 2, 11, 'relu'));
      Layer_Def.ADD(CreateOpt_Pool('pool', 2, 2));

      // ganze Bereiche 11x11
      Layer_Def.ADD(CreateOpt_Conv('conv', 15, 25, 2, 15, 'relu'));
      Layer_Def.ADD(CreateOpt_Pool('pool', 2, 2));
      Layer_Def.ADD(CreateOpt_Hidden('softmax', 'softmax', 100, 'NONE'));
      Net.makeLayers(Layer_Def);
      ...
      sWeightsFilename := '.\Weights.net';
      Net.sWightsFilename := sWeightsFilename;

      // Bilder laden
      iImageBlock := 1;
      Class_Imaging := TClass_Imaging.create;
      sDataPath := '.\Data\cifar-100-binary\';

      Class_Imaging.LoadCategories(sDataPath + 'coarse_label_names.txt',
        sDataPath + 'fine_label_names.txt');

      Class_Imaging.StartTrainData(sDataPath + 'train.bin');
      Class_Imaging.StartTestData(sDataPath + 'test.bin');

      // Class_Imaging.LoadCifar100_ADDTrainData(sDataPath + 'train.bin');
      // Class_Imaging.LoadCifar100_ADDTestData(sDataPath + 'test.bin');

      ResultArray := TMyArray.create(100); // es gibt 100 Klassen
      ...
      Options.learning_rate := 0.01; // 0.0001; //
      Options.momentum := 0.9; // 0.9
      Options.l1_decay := 0;
      ...
      Options.ChunkEnabled := false;
      Options.ChunkSize := 10 * Options.batch_size;
      // Anzahl der zu lernenden Bilder
      Options.ChunkRepetitions := 4; // Anzahl der Wiederholungen
      Options.ChunkAccLikeliHood := 0.8;
      // Mindest-W-keit, dass eine Information als gelernt akzeptiert wird
      Options.ChunkNonAccLikeliHood := 0.2;
      // Mindest-W-keit, dass eine Information als gelernt akzeptiert wird
    end;
  ...
  Try
    ...
    Begin
      Try
        ...
        PredictList := LearnThread.Net.getPrediction(5);
